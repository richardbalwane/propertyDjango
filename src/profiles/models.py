...
class Profile(models.Model):
    ...
    def send_activation_email(self):
        if not self.activated:
            ...
            sent_mail = False #send_mail() #Comment out since sender email not setup
            return sent_mail
...
def post_save_user_receiver(sender, instance, created, *args, **kwargs):
    if created:
        profile, is_created = Profile.objects.get_or_create(user=instance) # Defining word, profile
        default_user_profile = Profile.objects.get_or_create(user__id=1)[0] # user__username
        default_user_profile.followers.add(instance) # newly created user following the default profile
        #default_user_profile.followers.remove(instance) # we can remove things like this
        #default_user_profile.save() #with ManyToMany, we don't have to do this.
        profile.followers.add(default_user_profile.user)
        profile.followers.add(2)
        # We can now incorporate this in some view with toggle buttons... end points...
